...
CREATE TABLE `tblmatricula` (
  ...
  `fkEstudante` binary(16) NOT NULL,
  `fkTurma` binary(16) NOT NULL,
  `enumStatus` enum('Deferido','Indeferido') NOT NULL,
  `dataCriacao` datetime NOT NULL,
  `dataModificacao` datetime NOT NULL,
  PRIMARY KEY (`upkChave`),
  KEY `idx_tblmatricula_upkChave` (`upkChave`),
  KEY `fk_estudante_matr` (`fkEstudante`),
  KEY `fk_turma_matr` (`fkTurma`),
  CONSTRAINT `fk_estudante_matr` FOREIGN KEY (`fkEstudante`) REFERENCES `tblestudante` (`upkChave`),
  CONSTRAINT `fk_turma_matr` FOREIGN KEY (`fkTurma`) REFERENCES `tblturma` (`upkChave`)
